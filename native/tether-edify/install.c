...
char* WhiteListMacsFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    while(fgets(buffer, sizeof(buffer), macs) && returncode == 0) {
      ...
      sprintf(command,"/data/data/og.android.tether/bin/iptables -t nat -I PREROUTING -m mac --mac-source %s -j ACCEPT", buffer);
...
char* GetCfgFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    char* buffer = NULL;
    char* key;
    char *filename = "/data/data/og.android.tether/conf/tether.conf";
